...
namespace api_opendata.Service
{
    public class RepositoryService
    {
        ...
        public RepositoryService(DatabaseContext context, IMapper mapper, IHttpContextAccessor httpContext, UserManager<AspNetUsers> userManager)
        {
            _context = context;
            _mapper = mapper;
        ...
        {
            var item = await _context.Repository!.Where(x => x.IsDeleted == false).ToListAsync();

            var departmentDtos = _mapper.Map<List<RepositoryDto>>(item);

            return departmentDtos;
        }

        public async Task<int> SaveAsync(RepositoryDto dto)
        {
            int id = 0;
            var currentUser = await _userManager.GetUserAsync(_httpContext.HttpContext!.User);
            Repository item = null; // Declare item variable

            // Retrieve an existing item based on Id or if dto.Id is 0
            var existingItem = await _context.Repository!.FirstOrDefaultAsync(d => d.Id == dto.Id && d.IsDeleted == false);

            if (existingItem == null || dto.Id == null)
            {
                // If the item doesn't exist or dto.Id is 0, create a new item
                item = _mapper.Map<Repository>(dto);
                item.IsDeleted = false;
                item.CreatedTime = DateTime.Now;
                item.CreatedUser = currentUser != null ? currentUser.UserName : null;
                _context.Repository!.Add(item);
            }
            else
            {
        ...
        public async Task<bool> DeleteAsync(int Id)
        {
            ...
            var existingItem = await _context.Repository!.FirstOrDefaultAsync(d => d.Id == Id && d.IsDeleted == false);
            var currentUser = await _userManager.GetUserAsync(_httpContext.HttpContext!.User);
            ...
            _context.Repository!.Update(existingItem);
