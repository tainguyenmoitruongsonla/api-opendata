...
namespace api_opendata.Service
{
    public class StaffService
    {
        ...
        public StaffService(DatabaseContext context, IMapper mapper, IHttpContextAccessor httpContext, UserManager<AspNetUsers> userManager)
        {
            _context = context;
            _mapper = mapper;
        ...
        {
            var item = await _context.Staff!.Where(x => x.IsDeleted == false).ToListAsync();

            var departmentDtos = _mapper.Map<List<StaffDto>>(item);

            return departmentDtos;
        }

        public async Task<int> SaveAsync(StaffDto dto)
        {
            int id = 0;
            var currentUser = await _userManager.GetUserAsync(_httpContext.HttpContext!.User);
            Staff item = null; // Declare item variable

            // Retrieve an existing item based on Id or if dto.Id is 0
            var existingItem = await _context.Staff!.FirstOrDefaultAsync(d => d.Id == dto.Id && d.IsDeleted == false);

            if (existingItem == null || dto.Id == null)
            {
                // If the item doesn't exist or dto.Id is 0, create a new item
                item = _mapper.Map<Staff>(dto);
        ...
        public async Task<bool> DeleteAsync(int Id)
        {
            ...
            var existingItem = await _context.Staff!.FirstOrDefaultAsync(d => d.Id == Id && d.IsDeleted == false);
            var currentUser = await _userManager.GetUserAsync(_httpContext.HttpContext!.User);
            ...
            _context.Staff!.Update(existingItem);
