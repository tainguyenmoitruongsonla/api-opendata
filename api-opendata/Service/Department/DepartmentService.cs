...
namespace api_opendata.Service
{
    public class DepartmentService
    {
        ...
        public DepartmentService(DatabaseContext context, IMapper mapper, IHttpContextAccessor httpContext, UserManager<AspNetUsers> userManager)
        {
            _context = context;
            _mapper = mapper;
        ...
        {
            var item = await _context.Department!.Where(x => x.IsDeleted == false).ToListAsync();

            var departmentDtos = _mapper.Map<List<DepartmentDto>>(item);

            return departmentDtos;
        }

        public async Task<int> SaveAsync(DepartmentDto dto)
        {
            int id = 0;
            var currentUser = await _userManager.GetUserAsync(_httpContext.HttpContext!.User);
            Department item = null; // Declare item variable

            // Retrieve an existing item based on Id or if dto.Id is 0
            var existingItem = await _context.Department!.FirstOrDefaultAsync(d => d.Id == dto.Id && d.IsDeleted == false);

            if (existingItem == null || dto.Id == null)
            {
                // If the item doesn't exist or dto.Id is 0, create a new item
                item = _mapper.Map<Department>(dto);
        ...
        public async Task<bool> DeleteAsync(int Id)
        {
            ...
            var existingItem = await _context.Department!.FirstOrDefaultAsync(d => d.Id == Id && d.IsDeleted == false);
            var currentUser = await _userManager.GetUserAsync(_httpContext.HttpContext!.User);
            ...
            _context.Department!.Update(existingItem);
